...
function updateResultBadges(data) {
  ...
  headings.forEach((h3, domIndex) => {
    ...
    badge.setAttribute(
        'aria-label',
        `Click to email accessibility report for ${result.grade} grade, score ${
            result.score}`);
...
// Function to show report generation progress - Similar design to analysis
// progress
...
function hideReportGenerationProgress() {
  const progressIndicator =
      document.getElementById('gradeable-report-progress-indicator');
  if (progressIndicator) {
    progressIndicator.style.transform = 'translateX(320px)';
    setTimeout(() => {
      progressIndicator.remove();
